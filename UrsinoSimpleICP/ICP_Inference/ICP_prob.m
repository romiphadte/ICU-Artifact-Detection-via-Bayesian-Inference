...
I_next=(I_curr==(randn<=I_same_state))

Ca_next = Ca_curr + delT/tau*(-Ca_curr+sigma_Gx_curr) + 1.0*randn(); % for DBN, there should be some additive noise
Pic_next = delT*kE*Pic_curr/(1+Ca_curr*kE*Pic_curr)*...
    (   Ca_curr*(Pa_next-Pa_curr)/delT + ...
    (Ca_next-Ca_curr)/delT*(Pa_curr-Pic_curr) + ...
    (Pc_curr-Pic_curr)/Rf - ...
    (Pic_curr-Pvs_curr)/Ro + I_curr*drain_constant) + Pic_curr + 1.0*randn() ; % for DBN, there should be some additive noise
